...
const sortProducts = (products, sortKey) => {
  ...
  switch (sortKey) {
    ...
    case 'price-asc':
      return sorted.sort((a, b) => getDiscountedPrice(a.price, a.discount) - getDiscountedPrice(b.price, b.discount))
    case 'price-desc':
      return sorted.sort((a, b) => getDiscountedPrice(b.price, b.discount) - getDiscountedPrice(a.price, a.discount))
    default:
      return sorted
  }
}
...
const Products = () => {
  ...
  const { data: categorize } = useFetch(getCategories, [])
        ...
        <div className="row box-products mgt-32">
          <div className="c-12 m-12 l-2">
            <div className='category-products'>
              <span><i className="fa-solid fa-list"></i> Tất cả danh mục</span>
              <ul className="category-list">
                <li className="category-item">
                  <button type="button" className={category === null ? 'active' : ''} onClick={() => setCategory(null)}>Tất cả sản phẩm</button>
                </li>
                {
                  categorize.map((value) => {
                    return (
                      <li className="category-item" key={value.id}>
                        <button type="button" className={category === value.id ? 'active' : ''} onClick={() => setCategory(value.id)}>{value.categorize}</button>
                      </li>
                    )
                  })
                }
          ...
          <div className="c-12 m-12 l-10">
            {
              searchTerm && (
                <div className="search-summary">
                  Kết quả tìm kiếm cho "<strong>{searchTerm}</strong>" ({sortedProducts.length} sản phẩm)
            ...
            <div className="row products-control">
              ...
              <div className="paginate">
                ...
                <button type="button" className="prev btn" disabled={currentPage <= 1} onClick={() => setPage(currentPage - 1)}>
                  <i className="fa-solid fa-chevron-left"></i>
                </button>
                <button type="button" className="next btn" disabled={currentPage >= totalPages} onClick={() => setPage(currentPage + 1)}>
                ...
                <div className="row list-products">
                  ...
                  }
                  {
                    visibleProducts.map((product) => (
                      <ProductGridCard key={product.id} product={product} cardClassName="c-6 m-4 l-2-4 box-list-products" />
                    ))
                  }
                </div>
              </div>
