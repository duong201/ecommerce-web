...
describe('RegisterUser', () => {
  ...
  it('registers a new customer-level account and redirects to login', async () => {
    jest.mocked(registerUser).mockReturnValue(mockApiResponse({ status: 'success', id: 3 }))
  ...
  it('shows the server error message when registration fails (e.g. duplicate username)', async () => {
    jest
      .mocked(registerUser)
      .mockReturnValue(mockApiResponse({ status: 'error', message: 'Username đã tồn tại' }))
  ...
  it('shows a generic error message when the request itself fails', async () => {
    jest.mocked(registerUser).mockReturnValue(Promise.reject(new Error('network down')))
