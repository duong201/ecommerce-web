...
describe('OrderSuccess', () => {
  ...
  it('shows the order count and total when passed via navigation state', () => {
    render(
      <MemoryRouter initialEntries={[{ pathname: '/order-success', state: { orderCount: 2, total: 300000 } }]}>
        <OrderSuccess />
      </MemoryRouter>
    )
  ...
  it('links to the user\'s orders page only when logged in', () => {
    ...
    expect(screen.getByRole('link', { name: 'Xem đơn hàng của tôi' })).toHaveAttribute('href', '/user/info/2')
  })
})
