...
import { buildCartItem, buildUser } from '../../../test-utils/fixtures'
...
describe('Checkout', () => {
  beforeEach(() => {
    ...
    jest.mocked(getUser).mockReturnValue(mockApiResponse(user))
  })
  ...
  it('shows an error message for an invalid coupon', async () => {
    ...
    rejection.catch(() => {})
    jest.mocked(applyCoupon).mockReturnValue(rejection)
    renderWithRouter(<Checkout />)
    await screen.findByText('Áo thun nữ basic')
  ...
  it('places one order per cart item, clears the cart, then redirects to the success page', async () => {
    jest.mocked(addOrder).mockReturnValue(mockApiResponse({ status: 'success', order: {} }))
    jest.mocked(clearUserCart).mockReturnValue(mockApiResponse({ status: 'success' }))
