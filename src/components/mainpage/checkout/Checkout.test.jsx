...
const carts = [
  { id: 1, iduser: '2', idproduct: 1, name: 'Áo thun nữ basic', imgPrimary: 'img1.jpg', price: 150000, discount: 10, amount: 2, color: 'Đen', size: '28' },
]
...
describe('Checkout', () => {
  ...
  it('prefills the address and phone from the user profile', async () => {
    ...
    await waitFor(() => expect(screen.getByDisplayValue('45 Trần Phú, Huế')).toBeInTheDocument())
  ...
  it('applies a coupon and reduces the total', async () => {
    applyCoupon.mockReturnValue(mockApiResponse({ status: 'success', coupon: { code: 'GIAM10' }, discount: 27000 }))
    renderWithRouter(<Checkout />)
    await waitFor(() => expect(screen.getByText('Áo thun nữ basic')).toBeInTheDocument())
    ...
    await waitFor(() => expect(screen.getByText(/Đã áp dụng mã/)).toBeInTheDocument())
  ...
  it('shows an error message for an invalid coupon', async () => {
    const rejection = Promise.reject({ response: { data: { message: 'Mã giảm giá không hợp lệ' } } })
    rejection.catch(() => {})
    ...
    await waitFor(() => expect(screen.getByText('Mã giảm giá không hợp lệ')).toBeInTheDocument())
  })
